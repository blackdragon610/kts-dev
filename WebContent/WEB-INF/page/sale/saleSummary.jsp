...
-->
	...
	$(function() {
		...
		/* 他社製品検索の場合に品番のテキストボックス(FROMとTO)を入力不可にする  */
		 checkflg = false;
		$("#itemCodeAreaFlg").click(function(){
			if(checkflg == false){
		...
		$("#downloadSummary").click(function() {
			...
			if ($("#shipmentDateFrom").val() && $("#shipmentPlanDateTo").val()){
				fromAry = $("#shipmentPlanDateFrom").val().split("/");
				toAry = $("#shipmentPlanDateTo").val().split("/");
				fromDt = new Date(fromAry[0], fromAry[1], fromAry[2]);
				toDt = new Date(toAry[0], toAry[1], toAry[2]);
				if (fromDt > toDt) {
					alert("期間(出荷予定日) の出力開始日が、出力終了日より後の日付になっています。");
					return false;
				}
			}
			...
			goTransaction('saleSummaryDownLoad.do');
		});
	...
	<html:form action="/initSaleSummary" >

	<h4 class="heading">売上統計</h4>
	...
	<nested:nest property="saleSearchDTO" >
		...
		<table class="searchOptionTable">
			...
			<tr>
				<td>出荷予定日</td>
				<td><nested:text property="shipmentPlanDateFrom" styleId="shipmentPlanDateFrom" styleClass="calender" /> ～ <nested:text property="shipmentPlanDateTo" styleId="shipmentPlanDateTo" styleClass="calender" /></td>
			</tr>
			<tr>
				<td>法人</td>
			...
			<tr>
				<td>検索対象</td>
				<td>
					<label><nested:checkbox property="itemCodeAreaFlg" styleId="itemCodeAreaFlg" value="1" > 他社製品のみ </nested:checkbox></label>
					<input type="hidden" name="saleSearchDTO.itemCodeAreaFlg" value="0"/>
				</td>
			</tr>
			<tr>
				<td>品番（前方一致）</td>
