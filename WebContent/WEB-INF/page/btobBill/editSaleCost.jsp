 ...
 */
			...
			console.log(sysSalesItemId);
			console.log(sysSalesSlipId);
			console.log(cost);
			console.log(kindCost);
			console.log(itemRateOver);
			console.log(listPrice);
			console.log(costCheckFlag);
...
//					goTransaction("editsaleCost.do");
				...
				var domePostage = $(".domePostage").eq(idx).children('input').val();
				domePostage = new String(domePostage).replace(/,/g, "");
				while (domePostage != (domePostage = domePostage.replace(/^(-?\d+)(\d{3})/, "$1,$2")));
				$('.domePostage').eq(idx).html(domePostage + "&nbsp;円");

				$('.domePostageKind').eq(idx).html(domePostage + "&nbsp;円");
