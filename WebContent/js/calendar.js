...
function wrtCalendar(e,oj,flg,arg1,arg2){
  ...
  ddata += '  td.sunday    {background-color: rgba(255,0,0,0.6);}\n';
  ddata += '  td.saturday    {background-color: rgba(0,0,255,0.6);}\n';
...
if(!Moz || Fir){
  ...
  ddata += '<form>\n';
  ddata += '  <table border="2" bgcolor="' + c_table + '" bordercolor="' + c_table + '" width="140" height="140">\n';
  ...
  for(j=0;j<6;j++){
    ...
    for(i=0;i<7;i++){
      ...
      if(wrtyyyymm != nowyyyymm){
        ddata += '      <td style="opacity:0.2;" bgcolor="' + c_datex + '" width="14" height="14" align="center" valign="middle">';
        ddata += wrtdateA;
        ...
        ddata += '      <td ';
        if(syuku || !i) ddata += 'class="sunday" ';       // 日祝日
        if(!syuku && i == 6) ddata += 'class="saturday"';  // 土曜日
  ...
  ddata += '  <table border="0" width="200">\n';
  ddata += '<tr>\n';
  ddata += '<td align="center"><input type=button VALUE="今月表示"';
  ddata +=                'onClick="self.opener.wrtCalendar(0,self.opener.document.';
  ddata +=                 oj.form.name+'.'+oj.name+',\''+flg+'\',0,0)">';
  ddata += '    </td>\n';
  ddata += '    </tr>\n';
  ddata += '    </table>\n';
 ...
 *
 *         ・元号を改める政令によると平成31年は04月30日まで、平成元年は5月1日から
 *           よって、当スクリプトでは 2018/5/1 以降を「令和」と表示する。
 *
 ***************************************************************************************************/
...
function makeDate(inpt,year,month,day,i){
  ...
  if(inpt.match(/g/i)){
	if (year > 2018 && (month + '/' + day) > '4/30') {
      year = year - 2018;
      if(year == 1) year = '元';
      if     (inpt.match(/ggg/i)) year = '令和'+ year;
      else if(inpt.match(/gg/i))  year = '令'+ year;
      else if(inpt.match(/g/i))   year = 'R'+ year;
